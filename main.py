...
def detect_algae_impurities(image_path, roi_fraction=0.99):
    ...
    plt.title(f"Algae-like Impurities Detected: {len(keypoints) + len(valid_contours)}")
    plt.axis('off')
    plt.show()
...
# Example usage
image_path = 'water_samples/river_water_1.jpg'  # Path to the uploaded image
impurity_count = detect_algae_impurities(image_path)
print(f"Number of algae-like impurities detected: {impurity_count}")
